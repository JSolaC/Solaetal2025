...
model_data <- read.csv("MF_Model_data.csv") %>% #[1:100,]
  dplyr::select(TimeSeriesID, Binomial, Latitude, Longitude, Site, Units3, Units_class, Region, Year, Pop_size_LR) %>%
  mutate(
...
model_formula <- bf(
  Pop_size_LR ~                                        
    ...
    (-1 + Year | TimeSeriesID) +  
    (-1 + Year | Site:Binomial:Units3) + 
    (1|Site/Region),                   
  sigma ~ s(Year, k=4, bs="cr") + (1 | Units3),                                  
...
fit_single_model <- function(dataset, priors, model_formula, iterations = 2000) {
  ...
  # Step 7: Fit the model using brms
...
# Save the result
saveRDS(Bayesian_model, "MF_CD_timeseries.RDS")
