...
model_data <- read.csv("MI_Model_data.csv") %>% #[1:100,]
  dplyr::select(TimeSeriesID, Binomial, Latitude, Longitude, Site, Region, Year, Units3, Units_class, Pop_size_LR) %>%
  mutate(
...
# Save the result
saveRDS(Bayesian_model, "MI_timeseries.RDS")
